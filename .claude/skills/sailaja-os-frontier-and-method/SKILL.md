...
are now also DONE** (all 2026-07-21), and so is all of Item 5 — "Live
counts", "Real Recent Sessions", and (last to land, same day) the
schedule-aware dashboard, which turned out not to need the new data model
originally assumed. **Every roadmap item is now DONE.**
...
| Attendance | Implied by session history cards | Still no dedicated feature — **by design**, unchanged from the original plan: a session record (now real) with `studentId`+`date` IS the attendance record. **Fixed 2026-07-21**: the CBSE page's 3 "Recent Sessions" cards (the only curriculum pages that have one) render real session data, and the dashboard's "Upcoming Sessions" card now renders the real next 5 classes computed from each student's schedule (Item 5's schedule-aware-dashboard sub-item, DONE). |
...
| Schedule | "Weekly Master Schedule" table | Unchanged. Hardcoded static table; "+ Schedule Class" opens the now-functional add-session modal, but nothing populates the schedule table from real data yet. (Not to be confused with the dashboard's own schedule-awareness, which IS live now — see the Attendance row and Item 5.) |
| Exams / Lessons / Quiz bank | Add modals with Save buttons | **DONE 2026-07-21** (persistence via Phase 3(d), then edit/delete added same day on explicit owner request: "add edit/delete for lessons/exams/quiz"). All three now support add, edit, and confirm-gated delete, reusing the same modal for add/edit. Verified `verify-content-edit-delete.mjs`, 63/63 PASS. |
| Counts everywhere | "14 students", "9 classes this week" | **Fully fixed 2026-07-21**: nav badges, dashboard stat cards, page subtitles, and the Students-page filter-bar pills all read the store live (Item 5, "Live counts"). "N classes this week · M exams coming up" is also computed live now (Item 5, schedule-aware dashboard) — the static "9 classes this week..." text that ships in the raw HTML is only ever a placeholder, immediately overwritten by `renderScheduleAwareDashboard()` on load, same as every other live-counts element's static starting number. |
...
|---|---|---|
| ~~Schedule-aware dashboard~~ | **DONE 2026-07-21.** Turned out not to need a new data model after all — `scheduleDayNum()` parses the day-of-week straight out of the existing `schedule` string (`"Ddd · <time>"`, a fixed shape since `initDatabase()` first existed; no migration, no new key), and exams already carried a real `examDate`. `#dashboard-subgreeting` now reads `"N classes this week · M exams coming up"` computed live (the fabricated "all curricula on track" clause was dropped — no data backs a curriculum-wide sentiment claim); the "This Week" stat card is relabeled "Classes remaining" (today through Saturday, not "everyone's weekly total" — a class already held earlier this week doesn't count); the "Upcoming Sessions" card renders the real next 5 classes, soonest first. The dedicated "Schedule" page (Weekly Master Schedule table) stays static — out of scope, a separate future rebuild. | Milestone met: dashboard counts computed from stored schedules match a hand-counted seed, verified `verify-schedule.mjs` |
...
| ~~Lesson-plan / exam / quiz persistence~~ | **DONE 2026-07-21**, built on explicit owner request. `teach_os_lessons`/`teach_os_exams`/`teach_os_quiz` all live. **Edit/delete added later the same day**, also on explicit owner request ("add edit/delete for lessons/exams/quiz") — no longer add-only. Verified: `verify-content-crud.mjs` (26/26, add+reload) and `verify-content-edit-delete.mjs` (63/63, edit/delete/cancel+reload) | Milestone met — see `sailaja-os-daily-use-campaign` Phase 3(d) |
...
- Lesson/exam/quiz edit+delete present (DONE 2026-07-21 — expect hits): `grep -n "function openEditLessonModal\|function openEditExamModal\|function openEditQuizModal\|function deleteLesson\|function deleteExam\|function deleteQuizQuestion" index.html`
...
- Schedule-aware dashboard present (Item 5 fully DONE 2026-07-21 — expect a hit): `grep -n "function renderScheduleAwareDashboard" index.html`. Note: `grep -c "9 classes this week" index.html` will still find 1 hit — that's the static HTML placeholder inside `#dashboard-subgreeting`, overwritten at runtime on every load (same pattern as every other live-counts element's static starting number, e.g. the nav badge's literal "14") — its presence is NOT evidence the fix regressed.
...
- Schedule-aware dashboard alive end-to-end: `PW_PATH=<...> node .claude/skills/sailaja-os-browser-verification/scripts/verify-schedule.mjs` (expect `16/16 PASS`)
- Lesson/exam/quiz edit+delete alive end-to-end: `PW_PATH=<...> node .claude/skills/sailaja-os-browser-verification/scripts/verify-content-edit-delete.mjs` (expect `63/63 PASS`)
